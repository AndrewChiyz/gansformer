...
def to_2d(x, mode):
    ...
    if len(shape) == 2:
        return x
    if mode == "last":
        # [Bs, components, latent_dim] -> [BSxcomponent, latent_dim]
...
def dense_layer(
    x, dim, gain=1, use_wscale=True, lrmul=1, weight_var="weight", name=None
):
    if name is not None:
...
def apply_bias_act(
    x, act="linear", alpha=None, gain=None, lrmul=1, bias_var="bias", name=None
):
    if name is not None:
        bias_var = "{}_{}".format(bias_var, name)
    b = (
        tf.get_variable(
            bias_var, shape=[get_shape(x)[1]], initializer=tf.initializers.zeros()
        )
        * lrmul
    )
...
def norm(x, norm_type, parametric=True):
    if norm_type == "instance":
        x = tf.contrib.layers.instance_norm(
            x, data_format="NCHW", center=parametric, scale=parametric
        )
...
def att_norm(x, num, integration, norm):
    ...
    if integration in ["mul", "both"]:
        x *= tf.rsqrt(
            tf.reduce_mean(tf.square(x), axis=norm_axis, keepdims=True) + 1e-8
        )
...
def minibatch_stddev_layer(x, group_size=4, num_new_features=1, dims=2):
    ...
    y = tf.reshape(
        x,
        [group_size, -1, num_new_features, shape[1] // num_new_features, shape[2]]
        + last_dims,
    )  # [GMncHW]
    ...
    y = tf.reduce_mean(
        y, axis=[2, 3] + ([4] if dims == 2 else []), keepdims=True
    )  # [Mn111]
...
def get_positional_embeddings(
    max_res, dim, pos_type="sinus", dir_num=2, init="uniform", shared=False
):
    embs = []
    initializer = (
        tf.random_uniform_initializer()
        if init == "uniform"
        else tf.initializers.random_normal()
    )
    for res in range(max_res + 1):
        with tf.variable_scope("pos_emb%d" % res):
            ...
            if pos_type == "sinus":
                ...
                emb = tf.get_variable(
                    name="emb", shape=[size, size, dim], initializer=initializer
                )
            else:  # pos_type == "trainable"
                xemb = tf.get_variable(
                    name="x_emb", shape=[size, int(dim / 2)], initializer=initializer
                )
                yemb = (
                    xemb
                    if shared
...
def nnlayer(x, dim, act, lrmul=1, y=None, ff=True, pool=False, name="", **kwargs):
    ...
    if y is not None:
        x = transformer_layer(from_tensor=x, to_tensor=y, dim=dim, name=name, **kwargs)[
            0
        ]
    ...
    **kwargs
    ...
    if len(get_shape(x)) > 2:
        if pooling == "cnct":
            ...
            x = tf.nn.avg_pool(
                x, pool_shape, pool_shape, padding="SAME", data_format="NCHW"
            )
            x = to_2d(x, "first")
    ...
    if resnet:
        ...
        for layer_idx in range(layers_num):
            with tf.variable_scope("Dense%d" % layer_idx):
                x = apply_bias_act(
                    dense_layer(x, dim, lrmul=lrmul), act=act, lrmul=lrmul
                )
                x = norm(x, norm_type)
...
def conv2d_layer(
    ...
    w = get_weight(
        [kernel, kernel, get_shape(x)[1], dim],
        gain=gain,
        use_wscale=use_wscale,
        lrmul=lrmul,
        weight_var=weight_var,
...
def modulated_conv2d_layer(
    ...
    gain=1,
    use_wscale=True,
    lrmul=1,  # Parameters for creating the convolution weight
    weight_var="weight",
...
# X shape is [NCHW]
def att2d_layer(
    x, dim, kernel, grid, up=False, down=False, resample_kernel=None, num_heads=4
):
    if num_heads == "max":
        num_heads = dim
    ...
    x_patches = tf.reshape(
        x_patches, [-1, kernel * kernel, depth]
    )  # [N * H * W, k*k, C]
    ...
    x_flat = transformer_layer(
        from_tensor=x_flat, to_tensor=x_patches, dim=dim, name="att2d", **kwargs
    )[0]
...
def process_input(t, t_pos, t_len, name):
    ...
    if len(shape) > 3:
        misc.error(
            "Transformer {}_tensor has {} shape. should be up to 3 dims.".format(
                name, shape
            )
        )
    elif len(shape) == 3:
        ...
        # len(shape) < 3, a single global latent code case
        if t_len is None:
            misc.error(
                "If {}_tensor has two dimensions, must specify {}_len.".format(
                    name, name
                )
            )
...
def gate_attention(
    att_probs, tensor, pos, batch_size, num_heads, from_len, to_len, name, gate_bias=0
):
    ...
    gate = tf.transpose(
        tf.reshape(gate, [batch_size, from_len, to_len, num_heads]), [0, 3, 1, 2]
    )
    att_probs *= gate
    return att_probs
...
def integrate(tensor, tensor_len, control, integration, norm):
    ...
    bias = gain = control = apply_bias_act(
        dense_layer(control, control_dim, name="out"), name="out"
    )
    if integration == "both":
...
def compute_assignments(att_probs):
    centroid_assignments = att_probs / (
        tf.reduce_sum(att_probs, axis=-2, keepdims=True) + 1e-8
    )
    # [B, N, F, T]
    # reduce_sum() -> [B, N, 1, T]
    # cs: [B, N, T, F]
    # axis = -2
    # Reduces input_tensor along the dimensions given in axis=-2.
    # If keepdims is true, the reduced dimensions are retained with length 1.
    # the dimension of axis will be reduced.
    centroid_assignments = tf.transpose(
        centroid_assignments, [0, 1, 3, 2]
    )  # [B, N, T, F]
    # if the output is [B, N, T, F]
    # thus input is [B, N, F, T] -> [B, N, T, F]
    return centroid_assignments
    # normalize the attention weights along the column-axis
    # as centorid_assigments weights, [B, N, F, T] -> [B, N, T, F]
...
def compute_centroids(
    _queries,  # [B*F, N*H]
    queries,  # [B*F, N*H]
    to_from,  # first time is none (first layer)
    to_len,  # to tesor components
    from_len,  # from tensor components
    batch_size,  # bs
    num_heads,  # num_heads
    size_head,  # feature length in each heads
    parametric,  # parametric=True
):

    dim = 2 * size_head  # enlarge the feature length : latent code length
    from_elements = tf.concat(
        [_queries, queries - _queries], axis=-1
    )  # cancate the postion embedings,
    from_elements = transpose_for_scores(
        from_elements, batch_size, num_heads, from_len, dim
    )  # [B, N, F, H] # NxH=feature length, head_size is enlarged to 2xhead_size
    ...
    if to_from is not None:
        ...
        # to_from is [B, N, T, F]
        # if T < new to_len, then upsample
        if get_shape(to_from)[-2] < to_len:
            s = int(math.sqrt(get_shape(to_from)[-2]))
            to_from = upsample_2d(
                tf.reshape(to_from, [batch_size * num_heads, s, s, from_len]),
                factor=2,
                data_format="NHWC",
            )
            to_from = tf.reshape(to_from, [batch_size, num_heads, to_len, from_len])
        # if F < new From_len, then upsample
        # for each direction of information control, only one side of data will be enlarged
        # in styleGAN model in progressively way
        if get_shape(to_from)[-1] < from_len:
            s = int(math.sqrt(get_shape(to_from)[-1]))
            to_from = upsample_2d(
                tf.reshape(to_from, [batch_size * num_heads, to_len, s, s]),
                factor=2,
                data_format="NCHW",
            )
            to_from = tf.reshape(to_from, [batch_size, num_heads, to_len, from_len])
    ...
    if to_from is None or parametric:
        if parametric:
            to_centroids = tf.tile(
                tf.get_variable(
                    "toasgn_init",
                    shape=[1, num_heads, to_len, dim],
                    initializer=tf.initializers.random_normal(),
                ),
                [batch_size, 1, 1, 1],
            )
            # [bs, num_heads, t_len, head_sizex2]
        else:
            to_centroids = apply_bias_act(
                dense_layer(queries, dim * num_heads, name="key2"), name="key2"
            )
            # [B*F, N*H] -> FC ->[B*F, N*H*2]
            # dim = Hx2
            to_centroids = transpose_for_scores(
                to_centroids, batch_size, num_heads, dim, dim
            )
            # ????? what is this?
...
def transformer_layer(
    ...
    from_tensor,
    to_tensor,  # The from/to tensors [batch_size, t_len, dim] or [batch_size * t_len, dim]
    from_len=None,
    to_len=None,  # The from/to tensors lengths (must be specified if from/to has 2 dims)
    from_pos=None,
    to_pos=None,  # The positional encodings of the form/to tensors (optional)
    from_gate=False,
    to_gate=False,  # Add sigmoid gate on from/to, so that info may not be sent/received
    ...
    name="",
):  # Layer variable_scope suffix

    # Validate input shapes and map them to 2d
    from_tensor, from_pos, from_shape, from_len, batch_size = process_input(
        from_tensor, from_pos, from_len, "from"
    )
    # from_tensor has been reshaped as 2D tensor: [batch_size*from_len, dim]
    # so as the to_tensor                         [batch_size*to_len, dim]
    to_tensor, to_pos, to_shape, to_len, _ = process_input(
        to_tensor, to_pos, to_len, "to"
    )
    ...
    with tf.variable_scope("AttLayer_{}".format(name)):
        ...
        queries = apply_bias_act(
            dense_layer(from_tensor, dim, name="query"), name="query"
        )  # [B*F, N*H]
        keys = apply_bias_act(
            dense_layer(to_tensor, dim, name="key"), name="key"
        )  # [B*T, N*H]
        values = apply_bias_act(
            dense_layer(to_tensor, dim, name="value"), name="value"
        )  # [B*T, N*H]
        _queries = queries
        ...
        if from_pos is not None:
            queries += apply_bias_act(
                dense_layer(from_pos, dim, name="from_pos"), name="from_pos"
            )
        if to_pos is not None:
            keys += apply_bias_act(
                dense_layer(to_pos, dim, name="to_pos"), name="to_pos"
            )

        if kmeans:
            from_elements, to_centroids = compute_centroids(
                _queries,  #  [B*F, N*H]
                queries,  #  [B*F, N*H]
                to_from,  # first time none
                to_len,  #
                from_len,
                batch_size,
                num_heads,
                size_head,
                parametric=not iterative,  # parametric=True
            )
            # from_elements: [B, N, F, H*2] # NxH=feature length, head_size is enlarged to 2xhead_size
            # to_centroids: first time [B, num_heads(N), t_len(T), head_sizex2(Hx2)]
            # to_centroids:  [B, N, T, H*2]

        # Reshape queries, keys and values, and then compute att_scores
        # Notation: B - batch_size, F - from_len, T - to_len, N - num_heads, H - head_size
        # size_head = int(dim / num_heads) size_head * num_heads = dim H*N
        values = transpose_for_scores(
            values, batch_size, num_heads, to_len, size_head
        )  # [B, N, T, H]
        queries = transpose_for_scores(
            queries, batch_size, num_heads, from_len, size_head
        )  # [B, N, F, H]
        keys = transpose_for_scores(
            keys, batch_size, num_heads, to_len, size_head
        )  # [B, N, T, H]
        ...
        for i in range(kmeans_iters):
            with tf.variable_scope("iter_{}".format(i)):
                if kmeans:
                    # kmeans_iter= 1
                    # thus if i>0 block will not be excuted?
                    if i > 0:
                        # Compute relative weights of different 'from' elements for each 'to' centroid
                        to_from = compute_assignments(att_probs)  # [B, N, T, F]
                        # as centorid_assigments weights, [B, N, F, T] -> [B, N, T, F]
                        # Given:
                        # 1. Centroid assignments of 'from' elements to 'to' centroid
                        # 2. 'from' elements (queries)
                        # Compute the 'to' respective centroids
                        to_centroids = tf.matmul(to_from, from_elements)  # [B, F,]
                        # [B, N, T, F], [B, N, F, 2H] - > [B,N,T,2H]
                        # compute the centroids to tensor from from_elements?
                    ...
                    w = tf.get_variable(
                        name="st_weights",
                        shape=[num_heads, 1, get_shape(from_elements)[-1]],
                        initializer=tf.ones_initializer(),
                    )  # num_heads, 1, Hx2
                    # matmul([B, N, F, H*2] * [num_heads, 1, Hx2], [B, N, T, H*2])
                    att_scores = tf.matmul(
                        from_elements * w, to_centroids, transpose_b=True
                    )
                    # [B, N, F, H2]*[B,N,T,2H]
                    # att_scores = [B, N, F, T]

                # Scale attention scores given head size (see BERT)
                att_scores = tf.multiply(
                    att_scores, 1.0 / math.sqrt(float(size_head))
                )  # [B, N, F, T]
                # (optional, not used by default)
                # Mask attention logits using att_mask (to mask some components)
                if att_mask is not None:
                    att_scores = logits_mask(
                        att_scores, tf.expand_dims(att_mask, axis=1)
                    )
                # Turn attention logits to probabilities (softmax + dropout)
                # return multi-head attention probs in the first iteration
                att_probs = compute_probs(att_scores, att_dp)  # [B, N, F, T]
                # compute_probs will randomly drop cells, and randomly drop columns

        # Gate attention values for the from/to elements
        if to_gate:
            att_probs = gate_attention(
                att_probs,
                to_tensor,
                to_pos,
                batch_size,
        ...
        # Compute relative weights of different 'from' elements for each 'to' centroid
        if kmeans:
            # att_probs should be [B, N, F, T]
            to_from = compute_assignments(att_probs)
            # # [B, N, T, F] actually the transpose of the att_prob
            # to_from store the normalized att_probs

        # Compute weighted-sum of the values using the attention distribution
        # values should be  [B, N, T, H]
        # matmut([B, N, F, T], [B, N, T, H]) get [B, N, F, H]
...
def merge_images(x, dlatents, k, type, same=False):
    ...
    def k_reshape(t):
        return tf.reshape(t, [-1, k] + get_shape(t)[1:])
    ...
    # Take only image of highest score
        ...
        scores = tf.one_hot(
            tf.math.argmax(scores, axis=1), k, axis=1
        )  # [B, k, 1, H, W]
    # Merge the images recursively using a falling-leaves scheme (see k-GAN paper for details)
        ...
        for d, weight in enumerate(score_list[1:]):  # Process the k images iteratively
            max_weight = tf.math.reduce_max(
                scores[:, : d + 1],
            )  # Compute image currently most "in-front"
            new_alphas = tf.sigmoid(
                weight - max_weight
            )  # Compute alpha values based on "distance" to the new image
            alphas = tf.concat(
                [alphas * (1 - new_alphas), new_alphas], axis=1
            )  # Compute recursive alpha values
        scores = alphas
    else:  # sum
    ...
    if dlatents is not None:
        scores = tf.reduce_mean(
            tf.reduce_mean(scores, axis=2), axis=[-2, -1], keepdims=True
        )  # [B, k, 1, 1]
        dlatents = tf.tile(
            tf.reduce_sum(dlatents * scores, axis=1, keepdims=True), [1, k, 1, 1]
        )  # [B, k, L, D]
...
def G_GANsformer(
    ...
    **kwargs
    ...
    if not is_training or (
        style_mixing is not None
        and not tflib.is_tf_expression(style_mixing)
        and style_mixing <= 0
    ):
        style_mixing = None
    if not is_training or (
        component_mixing is not None
        and not tflib.is_tf_expression(component_mixing)
        and component_mixing <= 0
    ):
    ...
    latents_num = k + int(
        kwargs.get("transformer", False)
    )  # k attention region-based latents + 1 global latent (like StyleGAN)
    ...
    def mixing(latents_in, dlatents, prob, num, idx):
        ...
        with tf.variable_scope("StyleMix"):
            ...
            dlatents = tf.where(
                tf.broadcast_to(idx < mixing_cutoff, get_shape(dlatents)),
                dlatents,
                dlatents2,
            )
        return dlatents
    ...
    imgs_out, maps_out = components.synthesis.get_output_for(
        dlatents,
        latent_pos,
        component_mask,
        is_training=is_training,
    ...
    imgs_out = tf.identity(
        imgs_out, name="images_out"
    )  # [batch_size, num_channels, H, W]
    maps_out = tf.identity(
        maps_out, name="maps_out"
    )  # [batch_size, k (components_num), layers_num, heads_num, H, W]
    ...
    if return_dlatents:
        ...
        ret += (
            dlatents,
        )  # [batch_size, dlatent_broadcast, k (components_num), dlatent_dim]
...
def G_mapping(
    ...
    **_kwargs
    ...
    latents_num = k + int(
        global_latent
    )  # k region-based latents + 1 global latent (like StyleGAN)
    ...
    if net_dim is None:
        ...
        if latent_pos is not None:
            latent_pos = apply_bias_act(
                dense_layer(latent_pos, net_dim, name="map_pos"), name="map_pos"
            )
    ...
    if label_size:
        with tf.variable_scope("LabelConcat"):
            ...
            l = tf.tile(
                tf.expand_dims(tf.matmul(labels_in, w), axis=1), (1, latents_num, 1)
            )
    ...
    if k == 0:
        ...
        if shared_dim > 0:
            ...
            x = tf.reshape(
                apply_bias_act(
                    dense_layer(x, k * net_dim, name="outmap"), name="outmap"
                ),
    ...
    if output_dim is not None:
        x = tf.reshape(
            apply_bias_act(
                dense_layer(to_2d(x, "last"), output_dim, name="map_end"),
                name="map_end",
            ),
...
def G_synthesis(
    ...
    **_kwargs
    ...
    # Set variables
    k = components_num  # component number?
    act = nonlinearity  # activation function
    latents_num = k + int(
        transformer
    ...
    # Get global latent (the last one) return the last one of latent codes
    # dlatents [bs, number_layers, componets, compoents_dim]
    ...
    latent_pos.set_shape(
        [k, dlatent_size]
    )  # set the latent position is [k, dlatent_dim] k is number of components, 1~k
    # position embedding aims to generating position encoded feature with the same size with the latent code
    ...
    grid_poses = get_positional_embeddings(
        resolution_log2, pos_dim, pos_type, pos_directions_num, pos_init
    )
    # Encodings for Local attention (not used by default) not used by default
    if local_attention:
    ...
    def layer(x, dlatents, layer_idx, dim, kernel, att_vars, up=False):
        ...
        if transformer and res >= start_res and res < end_res:
            ...
            x = tf.transpose(
                tf.reshape(x, [shape[0], shape[1], shape[2] * shape[3]]), [0, 2, 1]
            )
            ...
            x, att_map, att_vars = transformer_layer(
                from_tensor=x, to_tensor=dlatents, dim=dim, name="l2n", **kwargs
            )
            ...
            if res == img2img:
                ...
                x = transformer_layer(
                    from_tensor=x, to_tensor=x, dim=dim, name="n2n", **kwargs
                )[0]
    ...
    def block(x, res, dlatents, dim, att_vars, up=True):  # res = 3..resolution_log2
        ...
        with tf.variable_scope("Conv0_up"):
            x, dlatents, att_map1, att_vars = layer(
                x,
                dlatents,
                layer_idx=res * 2 - 5,
                dim=dim,
        ...
        with tf.variable_scope("Conv1"):
            x, dlatents, att_map2, att_vars = layer(
                x, dlatents, layer_idx=res * 2 - 4, dim=dim, kernel=3, att_vars=att_vars
            )

        if architecture == "resnet":
            with tf.variable_scope("Skip"):
                t = conv2d_layer(
                    t, dim=dim, kernel=1, up=up, resample_kernel=resample_kernel
                )
                x = (x + t) * (1 / np.sqrt(2))
    ...
    with tf.variable_scope("4x4"):
        ...
        with tf.variable_scope("Conv"):
            x, dlatents, att_map, att_vars = layer(
                x, dlatents, layer_idx=0, dim=nf(1), kernel=3, att_vars=att_vars
            )
    ...
    for res in range(3, resolution_log2 + 1):
        with tf.variable_scope("%dx%d" % (2 ** res, 2 ** res)):
            ...
            x, dlatents, _att_maps, att_vars = block(
                x, res, dlatents, dim=nf(res - 1), att_vars=att_vars
            )
            att_maps += _att_maps
            ...
            if merge and (not merged) and res >= m:
                with tf.variable_scope("merge%s" % (res)):
                    x, dlatents_in = merge_images(
                        x, dlatents_in, k, merge_type, merge_same
                    )
                merged = True
    ...
    def list2tensor(att_list):
        ...
        for att_map in att_list:
            ...
            att_map = tf.transpose(
                tf.reshape(att_map, [-1, s, s, k]), [0, 3, 1, 2]
            )  # [NCHW]
            ...
            att_map = tf.reshape(
                att_map, [-1, num_heads, k, resolution, resolution]
            )  # [NhkHW]
            maps_out.append(att_map)

        maps_out = tf.transpose(
            tf.stack(maps_out, axis=1), [0, 3, 1, 2, 4, 5]
        )  # [NklhHW]
        return maps_out
    ...
    **_kwargs
    ...
    def block(x, res, aggregators):  # res = 2..resolution_log2
        ...
        if transformer and res >= start_res and res < end_res:
            # Reshape image features for transformer layer [batch_size (B), img_feats (WH), feat_dim (C)]
            x = tf.transpose(
                tf.reshape(x, [batch_size, shape[1], shape[2] * shape[3]]), [0, 2, 1]
            )
            ...
            aggregators = transformer_layer(
                from_tensor=aggregators,
                to_tensor=x,
                dim=latent_size,
                name="n2l",
                **kwargs
            ...
            if img2img > 0 and res == img2img:
                ...
                kwargs["from_gate"] = kwargs["to_gate"] = img_gate
                x = transformer_layer(
                    from_tensor=x, to_tensor=x, dim=latent_size, name="n2n", **kwargs
                )[0]
        ...
        ksize = (
            1 if local_att else 3
        )  # for local_attention, change 3x3 convolution to linear mapping
        if local_att:
    ...
    aggregators = None
    if transformer:
        aggregators = tf.get_variable(
            name="aggregators",
            shape=[components_num, latent_size],
            initializer=tf.random_uniform_initializer(),
        )
        aggregators = tf.tile(
            tf.expand_dims(aggregators, axis=0), [batch_size, 1, 1]
        )  # [batch_size, k, dim]
    ...
    with tf.variable_scope("4x4"):
        ...
        with tf.variable_scope("Dense0"):
            x = apply_bias_act(
                dense_layer(x, dim=nf(0)), act=act
            )  # [batch_size, nf(0)]
        ...
        with tf.variable_scope("ComponentScores"):
            if mbstd_group_size > 1:
                with tf.variable_scope("MinibatchStddev"):
                    ...
                    aggregators = minibatch_stddev_layer(
                        aggregators, mbstd_group_size, mbstd_num_features, dims=1
                    )
                    aggregators = tf.transpose(aggregators, [0, 2, 1])
            ...
            with tf.variable_scope("Dense"):
                aggregators = apply_bias_act(
                    dense_layer(to_2d(aggregators, "last"), latent_size), act=act
                )  # [batch_size * k, dim]
